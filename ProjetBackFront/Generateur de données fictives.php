...
$imageDirectory = __DIR__ . '/public/uploads';
$imageFiles = array_values(array_diff(scandir($imageDirectory), ['.', '..'])); // Filtrer les fichiers inutiles

$data = [];

// Générer plusieurs utilisateurs avec plusieurs photos
for ($i = 0; $i < 500; $i++) { 
    ...
    for ($j = 0; $j < $maxPhotos; $j++) {
        $userPhotos[] = $imageFiles[array_rand($imageFiles)];
    }
    ...
    $data[] = [
        ...
        'images' => $userPhotos ?? [], // Liste de photos avec tableau vide par défaut
